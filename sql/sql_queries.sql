...
select rocket,count(*) as launch_count
from dbt_spacex.launches
group by rocket;


select rocket, avg(case when success = true then 1 else 0 end) as success_rate
from dbt_spacex.launches
group by rocket;
...
select l.id, l.launch_date,r.name as rocket_name, r.active
from dbt_spacex.launches l
join dbt_spacex.rockets r on l.rocket = r.id;

select * from dbt_spacex.launches
where launch_date = (select max(launch_date) from dbt_spacex.launches);
...
------- creating a view-----
create or replace view dbt_spacex.successful_launches as GROUP BY
select *
from dbt_spacex.launches
where success = true;
